use super::{AbstractSyntaxTree, Identifier, LetStatement, ReturnStatement, Statement};
...
impl Parser {
    ...
    fn parse_let_statement(&mut self) -> Result<Statement, Error> {
        ...
        let identifier: Identifier = Identifier::new(self.cur_token.clone())?;
...
mod tests {
    use super::Parser;
    use crate::{Lexer, parser::LetStatement, parser::Statement};
    ...
    fn verify_let_statement(statement: &Statement, expected_name: &str) -> bool {
        match statement {
            Statement::LetStatement(let_statement) => {
                ...
                assert_eq!(
                    let_statement.identifier.token.token_literal(),
